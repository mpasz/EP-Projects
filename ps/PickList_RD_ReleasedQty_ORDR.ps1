...
$SQL_PICK_LIST = "SELECT
 ""DocEntry""
FROM ORDR
WHERE IFNULL(""U_CreateSendOrder"",'N') = 'T'  ORDER BY ""DocEntry""" ;

$SQLQUERY_PICK_ORDER_LIST = "SELECT DISTINCT
T0.""DocEntry"" AS ""DocEntry"", T0.""ItemCode"" AS ""ItemCode"", T0.""LineNum"" AS ""LineNum"", T0.""WhsCode"", T0.""U_PickQty"" AS ""Quantity"", 
ROW_NUMBER()OVER(PARTITION BY T0.""DocEntry"" ORDER BY T0.""DocEntry"", T0.""LineNum"")-1 AS ""RowNum"",
T1.""DocEntry"" AS ""Ref"", IFNULL(T3.""USER_CODE"",'brak') AS ""USERID""
FROM RDR1 T0
 INNER JOIN ORDR t1 on t0.""DocEntry""=t1.""DocEntry""
 LEFT OUTER JOIN OUSR T3 ON T1.""UserSign"" = T3.""USERID""
    WHERE 
...
                  
                        ...
                        $message = $pickList.UpdateReleasedAllocation()
                     ...
                     #
                     #       $pickList.Lines.SetCurrentLine($j);
                     #       $qty = $pickList.Lines.PickedQuantity
                     #
                     #       $pickList.Lines.PickedQuantity = 0;
