...
function CreateMors($pfcCompany){
    ...
    $SQLQUERY_ITEMS_LIST = "select DL.""DocEntry"",D.""DocNum"",B.""U_ItemCode"", IFNULL(DL.""U_Revision"",B.""U_Revision"") AS ""Revision"", DL.""LineNum"", DL.""InvQty"" AS ""qty"" 
        from PDN1 DL inner join OPDN D ON D.""DocEntry"" = DL.""DocEntry"" 
        inner join ""@CT_PF_BOM1"" BL ON DL.""ItemCode"" = BL.""U_ItemCode""
        inner join ""@CT_PF_OBOM"" B ON BL.""Code"" = B.""Code""
        
        WHERE DL.""U_Niezaplanowane"" = 'Y' OR DL.""U_Niezaplanowane"" = 'Fxxx' ORDER BY DL.""DocEntry""";

    $SQLQUERY_MOR_DETAILS = "SELECT * FROM ""CT_LiniaPos""(@ItemCode,@Revision,@Iteration)";

    $SQLQUERY_UPDATE_STATUS = "UPDATE PDN1 SET ""U_Niezaplanowane"" = @Status WHERE ""DocEntry"" = @DocEntry AND ""LineNum"" = @LineNum";

    $SQLQUERY_CREATE_SSTU = "Call CreateSSTU_FROM_PZ (@DocEntry,@LineNum,@MorEntry)";
    ...
    $SQL_INSERT_LOG = "INSERT INTO ""@LOG_POWERSHELL"" (""Code"",""Name"",""U_Object"",""U_Key_Name"",""U_Key_Value"",""U_Remarks"",""U_Date"",""U_Time"",""U_Script_Name"",""U_Status"")
        VALUES (SUBSTR(SYSUUID,0,30), SUBSTR(SYSUUID,0,30),@Object,@Key_Name,@Key_Value,@Remarks,CURRENT_DATE, CAST(CONCAT(HOUR(CURRENT_TIME),MINUTE(CURRENT_TIME)) AS int), 'MorFromDelivery.ps1','F')";
            ...
            {           
                ...
                try {
                    ...
                    if($message -lt 0)
                    {
                        ...
                        $ms = [string]::Format("Zlecenie produkcyjne nie zostało dodane dla dokumentu PZ: {0}, Linia: {1}`n`nSzczegóły: {2}",$line.docNum,$line.lineNum,$err); 
                        $Object = '20';
                        $KeyName = 'DocEntry';
